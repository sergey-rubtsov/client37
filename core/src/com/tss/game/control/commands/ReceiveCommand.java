...
public class ReceiveCommand implements Command {
	...
	public void game(int seat, String[] playersId);
	...
	public void sync(int step, int currentSeat, int yourDice, int seat, String[] args);
	...
	if (comm.equals("from")) {
	    ...
	    listener.next(Integer.parseInt(array[1]), Integer.parseInt(array[2]));
	} else if (comm.equals("sync")) {	    
	    listener.sync(Integer.parseInt(array[1]), Integer.parseInt(array[1]), 0, 0, null);
	} else if (comm.equals("game")) {	    
	    listener.game(Integer.parseInt(array[1]), array[2].split(";"));
	}
